...
long seperate_textfile(char filename[], int processor_number, long lines) {
  ...
  for (i = 0; i < processor_number; ++i) {
    ...
    for (tim = tstart; tim <= tend; ++tim) {
      ...
      if (len >= 20) {
        printf("line %ld length is larger than SLEN, please make sure all line "
               "less than SLEN\n",
               tim + 1);
      }
      fprintf(fw, "%s\n", input);
...
long seperate_binaryfile(char filename[], int processor_number, long lines) {
  ...
  for (i = 0; i < processor_number; ++i) {
    FILE *fw =
        fopen(parda_generate_pfilename(filename, i, processor_number), "wb");
    tstart = parda_low(i, processor_number, sum);
    tend = parda_high(i, processor_number, sum);
...
long parda_seperate_file(char inputFileName[], int processor_number,
                         long lines) {
  if (lines == -1)
    lines = get_file_lines(inputFileName);
  int psize = processor_number;
  if (!is_binary)
    seperate_textfile(inputFileName, psize, lines);
  else
    seperate_binaryfile(inputFileName, psize, lines);
  char linesFile[50];
