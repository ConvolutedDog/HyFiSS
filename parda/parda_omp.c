...
processor_info_t parda_get_thread_info(long lines, long begin, int pid,
                                       int psize) {
...
void parda_omp_openfile(char inputFileName[], const int pid, const int nthreads,
                        const int psize, FILE *fpa[]) {
  int i;
  for (i = 0; i < nthreads; i++) {
    char *pfilename = parda_generate_pfilename(
        inputFileName, pid * nthreads + i, psize * nthreads);
    fpa[i] = fopen(pfilename, "r");
  }
}

program_data_t parda_omp_input_with_filename(char inputFileName[],
                                             program_data_t *pdt_a, long begin,
                                             long end, int pid, int psize) {
...
program_data_t parda_omp_input(char inputFileName[], program_data_t *pdt_a,
                               long begin, long end, int pid, int psize) {
  int nthreads = threads;
...
#pragma omp for
    for (i = 0; i < nthreads; i++) {
      ...
      parda_input_with_filename(parda_generate_pfilename(inputFileName,
                                                         pid * nthreads + i,
                                                         psize * nthreads),
                                &pdt_a[i], pit_a[i].tstart, pit_a[i].tend);
